...
function checksExistsUserAccount(request, response, next) {
  ...
  if (!user) {
    return response.status(404).json({ error: "User notfound" });
  }
...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  const freeLimit = user.todos.length;

  if (proPlan === false && freeLimit === 10) {
...
function checksTodoExists(request, response, next) {
  ...
  if (!isUuid) {
    return response.status(400).json({ error: "it's not uuid" });
  }
